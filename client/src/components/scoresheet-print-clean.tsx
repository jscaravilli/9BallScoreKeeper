...
export default function ScoresheetPrint({ match }: ScoresheetPrintProps) {
  ...
  function renderMatchDataMarkup() {
    ...
    const coordinates = {
      ...
      // Handicaps (targets)
      player1Target: { x: 885, y: 325, fontSize: '36px', fontWeight: 'bold', color: 'blue' },
      player2Target: { x: 885, y: 460, fontSize: '36px', fontWeight: 'bold', color: 'blue' },
    ...
    const dataPoints = [
      ...
      { key: 'player1Score', value: `${Math.min(match.player1Score, player1Target)}`, coords: coordinates.player1Score },
      { key: 'player2Score', value: `${Math.min(match.player2Score, player2Target)}`, coords: coordinates.player2Score },
