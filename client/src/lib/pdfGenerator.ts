...
export async function renderScoresheetToCanvas(
  ...
  if (matchData) {
    ...
    // Player names - limit to 18 characters, non-bold, smaller font
    const player1Text = matchData.player1Name.slice(0, 18);
    const player2Text = matchData.player2Name.slice(0, 18);
    ...
    // Handicaps (targets)
    ctx.fillText(matchData.player1Target.toString(), 885, 325); // [885,325] Player1 Handicap
    ctx.fillText(matchData.player2Target.toString(), 885, 460); // [885,460] Player2 Handicap
